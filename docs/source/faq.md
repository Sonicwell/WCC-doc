...
|419      | 呼入通话未匹配到任何Client，无法进行转向|例：呼入的DID和中继都不绑定client
 ...
 **注**：只有内线之间或外线号码呼入到被监控坐席时，才会触发该功能，指示灯才会变成红色，呼出到外线暂不支持该功能
 ...
 #### 13  不同通话类型，原始主叫号码、原始被叫号码等8个字段显示情况 .  
| 前提条件|测试坐席A/B/C|使用的外线号码|坐席组号码（设置号码前缀）|坐席组名称（设置名称前缀）|振铃组号码（设置号码前缀）|振铃组名称（设置名称前缀）|IVR菜单名称（设置号码前缀、名称前缀|did号码（设置号码、名称前缀）|  
|-----|-----|-------|------|-------|--------|---------|---------|-------|    
||2008/2006/2001|881005|20250701|20250701坐席组|444555|ring|test1|581000|  

| 显示字段/呼叫类型 |原始主叫号码  | 原始主叫名称   |实际主叫号码| 实际主叫名称 |接入号码  | 实际被叫号码  | 原始被叫号码  | 原始接入号码 |  
| ----------- | ----------- |-------|-------|-------|-------|-------|-------|-------|  
|内线（呼入到振铃组）|坐席工号 2008|2008|振铃组号码前缀+2008|振铃组前缀+坐席名称|振铃组号码 444555|2006|振铃组名称 ring|        |  
|呼出到外线|坐席工号 2001|    |外呼档案主叫号码替换 011888|外呼档案主叫名称替换 1106|881005|外呼档案被叫号码替换 881006|881005|       |  
|呼入到坐席|881005|     |did号码前缀+881005|did名称前缀|did号码 581000|2008|2008|did号码 581000|  
|呼入到坐席组|881005|      |did号码前缀+881005|did名称前缀|did号码 581000|2008|2008|did号码 581000|  
|呼入到振铃组|881005|     |坐席组号码前缀+did号码前缀+881005 |坐席组名称前缀+did名称前缀|did号码 581000|2006|坐席组号码 20250701|did号码 581000|  
|呼入到语音播报（转向到振铃组）|881005|     |did号码前缀+881005|did名称前缀|did号码 581000|2006|2006|did号码 581000|  
|呼入到条件（转向到振铃组）|881005|     |振铃组号码前缀+did号码前缀+881005|振铃组名称前缀+did名称前缀|did号码 581000|2006|振铃组名称 ring|did号码 581000|  
|呼入到IVR语音菜单（按1转向到振铃组）|881005|     |振铃组号码前缀+IVR号码前缀+did号码前缀+881005|振铃组名称前缀+IVR名称前缀+did名称前缀|did号码 581000|2006|振铃组名称 ring|did号码 581000|  
|预拨号|坐席组号码20250701|     |外呼档案主叫号码替换 011888|外呼档案主叫名称替换 1106|坐席组号码20250701|2006|881005|     |  

|总结|      |     |  
|--------------|----------|--------|     
|原始主叫号码与实际主叫号码的区别|原始主叫号码前面是没有任何号码前缀|  |   
||实际主叫号码在呼入通话类型中会加号码前缀，在呼出通话类型中会根据外呼档案的设置进行主叫号码替换，显示替换后的号码|    |  
|原始主叫名称与实际主叫名称的区别|原始主叫名称没有前缀|    |
||实际主叫名称是在呼入通话类型中会加名称前缀，在呼出通话类型中会根据外呼档案的设置进行主叫名称替换，显示替换后的名称 |   
|原始被叫号码与实际被叫号码的区别 |原始主叫号码是指最先转接到的层级的号码。|例如外线呼入，did转向到坐席组，那坐席组号码就是原始被叫号码 |   
||实际被叫号码是指最后接通电话的号码。|例：外线呼入，did转向坐席组，坐席A最后接通了电话，坐席A的工号就是实际被叫号码|  
||在外呼过程中，外呼档案可能会替换被叫号码，那么最后接通电话的是被替换的号码，被替换的号码就是实际被叫号码|  |  
|接入号码|是指拨打的号码（再通过其进行转接）。出是显示外线号码，呼入是一般为did号码|  
||若内线拨打振铃组、拨打坐席组号码等不走外呼档案、did号码时，接入号码为振铃组号码、坐席组号码。|