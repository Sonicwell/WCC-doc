...
> ***输入框***-用户可以输入数据但无法换行；  
>  
> ***电话号码***-填写一个电话号码，该字段只允许添加数字，数字长度应在2-15位；
>
> ***文本框***-用户可以输入数据也可以换行；
>
> ***邮件***-填写一个邮件地址，邮件地址格式example@example.com；
>
> ***日期***-选择一个日期进行填写,日期只能选择不能填写；
>
...
-   **唯一索引**：表里数据的当前字段没有重复值（客户包里有客户数据，在编辑界面，索引参数里的下拉里会出现唯一索引里已经选择的字段）。
...
-   **唯一标识**：为该客户包设置一个唯一标识，仅支持填写数字和字母对长度没有要求。
