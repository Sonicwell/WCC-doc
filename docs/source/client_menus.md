...
-   坐席组名称：坐席组的名称，当需要选择坐席组时，显示的选择名称。
...
-   **话后处理**：表示当通话结束后，系统将暂停坐席以便他能够完成剩余的工作而不是被新的通话打断。此处三个选项，①禁用：关闭话后处理功能---坐席不会进入话后的状态；②应答话后：当坐席收到呼入到坐席组或是预拨号的来电后，会进入话后的状态；③振铃话后：坐席电话振铃后，无论是否接通这次来电，都会进入话后的状态。
...
-   **点击拨号**：坐席可以在【我的客户】中的电话符号拨出电话进行双呼，双呼时不需要坐席先接通话机再进行外呼，而是可以直接自动接通再外呼。如果不选择自动应答，在点击呼叫后最先振铃的是坐席的话机，坐席需要先让话机接通才能继续向外拨号
...
标记坐席所在的部门可以让管理员更清楚地管理坐席。在呼叫流程中有一个抢答流程，系统规定只有两个坐席在同一个部门下，才能够对通话进行抢接。没有分配部门的坐席默认为默认部门，不显示在部门页面。
