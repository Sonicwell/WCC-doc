...
1. 核心内容：用于查看指定坐席组在所选任务中的详细统计数据。支持多维度筛选与灵活组合，便于全面分析坐席组的业务表现。
...
【任务】可单选或多选一个或多个任务，查看所选坐席组在这些任务下的表现数据。

【查看字段】自定义需要展示的统计指标（默认包含常用字段，也可按需勾选）。
...
1. 核心功能：集中展示各坐席组在指定时间范围内的整体运营数据，帮助管理者快速掌握团队级的服务效率与话务表现。
...
【坐席组】可单选或多选一个或多个坐席组进行对比分析。

【查看字段】自定义需要展示的统计指标（支持按需勾选）。
...
1. 核心功能：在部门报表页面，用户可以选择一个日期范围，选择想要查看的部门，点击搜索进行查看，此外，用户还可以选择该部门想要查看的坐席数据或是该部门的通话时长查看报表内容。
...
1. 核心功能：提供对坐席、坐席组及整体话务状态的实时可视化能力，帮助管理人员及时掌握运营动态，快速响应异常情况。

2. 坐席组坐席

此视图聚焦于指定坐席组内的坐席实时状态，支持输入一个或多个坐席组进行监控。

![alt text](_static/images/client/media/image302.png)

- 坐席状态统计（红框区域）展示该坐席组内各类状态的坐席数量及占比。
...
**暂停**：手动暂停或因队列规则自动暂停的坐席（如“未应答暂停”）。

**振铃**：正在响铃但尚未接通的呼入/呼出。

**通话**：通过队列呼入、预拨号或联系人双呼等方式建立的通话；若坐席通过其他方式进入通话，则显示为“忙碌中”。

**保持**: 仅队列通话中的“保持”状态会显示；非队列保持不展示。

**排队人数**: 当前在该坐席组队列中等待的客户数量。
...
强插 - 强行介入通话，形成三方通话（客户与坐席均可听到监控者声音）。
...
距上次通话时间- 距离上一次通话结束的时间。

显示字段自定义：用户可通过“显示字段”下拉菜单，灵活配置页面展示的列（见图示紫色框区域）。

![alt text](_static/images/client/media/image306.png)

- **异常提醒**：若有来电进入队列但无坐席应答，系统将弹出 绿色呼叫队列提示框，提醒管理人员及时干预。
...
展示系统中所有坐席的实时状态，默认包含全部坐席；勾选 “只显示 SIP 在线” 后，仅显示已注册话机的坐席。
...
适用于高并发场景，以 百分比形式 展示坐席状态分布（内部通话与转接通话不参与计算）。同样支持 “只显示 SIP 在线坐席” 的筛选选项，便于聚焦活跃坐席群体。

![alt text](_static/images/client/media/image311.png)  

#### 3.13.2 大屏监控

1. 核心功能：提供全局坐席状态的可视化看板，适用于指挥中心或运营大厅场景。
...
3. 筛选选项：可选择 仅显示 SIP 在线坐席，过滤离线终端。
...
1. 核心功能：系统自动记录所有用户操作行为日志，默认保留 30天。具备相应权限的客户端及以上用户，可在此查询并下载指定坐席在特定时间范围内的通话记录及相关客户数据，便于审计、问题追溯与服务质量分析。
...
【查看】点击可展开日志详情，查看完整操作内容、时间戳、参数及上下文信息。

日志数据涵盖关键操作行为，是系统安全审计与运营复盘的重要依据

#### 3.14.2 坐席事件

1. 核心功能：用于检索和审查坐席在系统中的操作行为记录。此功能支持按多种条件筛选日志，并允许用户自定义显示字段以满足不同的查看需求。
...
- **时区**:系统统一使用的时区，影响日志时间、报表统计、预约提醒等时间相关功能。

- **语言**:话机语音播报所使用的语言。当前支持：中文、英文、日语。

- **国家**:系统部署所在国家，用于匹配本地化规则（如号码格式、节假日等）。

- **区号**:系统默认电话区号，用于外呼号码补全及来电显示标准化。

- **日期格式模式**：控制日期和时间的显示方式：

        精确模式：允许自定义日期与时间格式（如 YYYY-MM-DD HH:mm:ss）

        友好模式：使用系统预设的简洁格式（如“今天 14:30”）。

- **时长格式**：通话或操作时长的显示方式：

      时:分:秒（如 01:25:30）  纯秒数（如 5130 秒）

- **号码格式**：设置电话号码的显示格式（例如带国家码、去零、加括号等），提升可读性与一致性。

- **称呼格式**：定义客户或坐席姓名的显示习惯（如“姓+名”、“名+姓”或带尊称等）。

- **预约提醒时间**：设置坐席预约、工单预约等任务的提前通知时间范围（如提前10分钟、30分钟等）。

- **自动绑定分机**：启用后，系统将为新坐席自动分配 SIP 分机号。勾选此项后，需同时选择 【默认话机部署模板】 以完成自动配置。

- **IP访问白名单**：仅允许列表中的 IP 地址访问系统，提升安全性。多个 IP 可用逗号或换行分隔。

***注意：修改部分配置（如时区、语言、号码格式）可能需要刷新页面或重新登录才能生效。***

2. 货币

![alt text](_static/images/client/media/image319.png)

- **货币**：选择系统使用的货币种类（如人民币 CNY、美元 USD、日元 JPY 等）。

- **货币符号**：设置货币对应的显示符号（如 ¥、 $ 、€ 等），用于金额前缀或后缀展示。

- **货币小数点**：指定小数部分的分隔符（通常为 . 或 ,），例如：123.45 或 123,45。

- **数字分隔符**：用于分隔整数与小数部分的符号（通常与“货币小数点”一致，部分系统保留此字段用于兼容）。

- **千位分隔符**：设置千位分组的分隔符（如 ,、. 或空格），例如：1,000,000 或 1.000.000

3. 数据

![alt text](_static/images/client/media/image320.png)

- **默认客户包:**系统在呼入或呼出时自动关联的默认客户数据包。若当前使用的客户包被设为“不可见”或失效，系统将自动回退至该默认客户包。

- **导出文件编码**：设置数据导出文件的字符编码格式（如 UTF-8、GBK 等），以确保在不同操作系统或软件中正确显示中文及其他特殊字符。

- **导出单文件最大行数**：限制单个导出文件包含的最大记录行数（例如 10,000 行）。当数据量超过此值时，系统将自动分卷生成多个文件，避免文件过大导致处理异常。

4. 拨号

![alt text](_static/images/client/media/image321.png)

- **拨打外线超时（秒）**：外呼至外部电话时，振铃等待对方接听的最大时长。超时后将自动挂断或进入预设处理流程。

- **拨打分机超时（秒）**：呼叫内部分机时的振铃超时时长。超过该时间未接听，系统将执行后续动作（如转语音信箱或提示忙音）。

- **盲转振铃超时时长（秒）**：执行盲转（即未咨询目标坐席直接转接）时，目标坐席振铃的最大等待时间。超时后可触发回呼等策略。

- **盲转未接通回呼**：启用后，若盲转的目标坐席未接听，系统将自动将通话回拨至发起盲转的原坐席，避免客户流失。

- **盲转回呼主叫名称前缀**：设置盲转回呼时显示在原坐席话机上的主叫名称前缀（例如“ [ 回呼 ] ”），便于坐席快速识别回呼来源。

- **默认等待音乐**：客户在接通前（如排队、保持状态）听到的默认背景音乐或提示音。可上传自定义音频文件。

- **默认外呼档案**当坐席发起外呼但未指定外呼档案时，系统将自动使用此默认档案；若未配置任何外呼档案，则采用系统内置的默认规则。

*** 建议 ***：

*** 外线/分机超时建议设为 20–30 秒，兼顾客户耐心与坐席效率；启用“盲转未接通回呼”可显著提升客户接通率；自定义等待音乐有助于提升品牌体验。***

5. 呼叫设置

![alt text](_static/images/client/media/image322.png)

- **外显号码轮询**：启用该功能后，系统将根据设定规则自动轮换外显（主叫）号码，适用于外呼接口、坐席外呼及外呼任务等场景。轮询模式说明：值为 1，每天切换一次外显号码；值为 0，每次通话均使用新的外显号码。

- **外显号码**：用于设置对外显示的主叫号码，即被叫方看到的来电号码。

- **黑名单**：可配置当呼叫对象命中黑名单时的处理策略，例如转接至特定流程或直接挂断。

#### 3.15.2 销售漏斗

1. 核心功能：用于记录坐席在与客户通话过程中所更新的客户状态。当销售漏斗配置生效（即“销售漏斗数量 > 0”）时，系统将在客户提交成功的位置自动记录对应的漏斗状态。

![alt text](_static/images/client/media/image323.png)

2. 关键操作

【默认】显示当前可选的客户包（即该销售漏斗适用的客户范围）。

【新增】创建一个新的销售漏斗规则。

【显示/隐藏字段】自定义本页面中需要展示或隐藏的字段，便于按需查看信息。
...
- **名称**：为该销售漏斗设置一个清晰、可识别的名称（例如：“意向客户跟进”）。

- **分值**：设置该漏斗阶段对应的分值，用于量化坐席的工作成果或绩效评估。
...
- **客户包**：指定该销售漏斗适用的客户分组（即仅对所选客户包中的客户生效）。

#### 3.15.3 后台任务

1. 核心功能：用于管理在系统中异步执行的耗时操作，例如数据导入、导出等。页面默认仅展示当前正在运行的任务。如需查看所有历史及当前任务，请点击 【全部任务】。

![alt text](_static/images/client/media/image325.png)

2. 关键操作

【搜索】支持通过任务编号或文件名称快速查找任务。

【显示/隐藏字段】自定义任务列表中需要展示或隐藏的列字段，便于按需查看关键信息。

【全部任务】切换至完整任务视图，包含进行中与已完成/失败的历史任务。

【请选择】按选择条件删除。

【删除】删除这个任务。

【下载】仅限发起该导入任务的用户下载生成的文件；其他用户无下载权限。

【下拉菜单】提供单条任务的快捷操作，包括 删除该任务 等选项。
...
- **api验证串**：用于身份鉴权的密钥字符串。调用系统 API 时需在请求中携带此验证串，以确保请求来源合法。

- **IP访问白名单**：限制仅允许指定 IP 地址访问该客户端。启用后，只有白名单中的 IP 才能使用账号密码登录或调用相关接口，有效提升安全性。。

2. API 限制

![alt text](_static/images/client/media/image327.png)

- **短信**：限制每日可通过 API 发送的短信数量。

- **通知**：限制每日可通过 API 触发的通知数量（如站内信、邮件等）。

- **获取数据**：限制每日通过 API 获取客户、通话等数据的请求次数。

- **点击呼叫**：限制每日通过 API 发起“点击拨号”操作的次数。

*** 建议：根据实际业务量合理设置限额，既保障系统稳定性，又避免影响正常集成使用。***
