...
-   名：坐席名，坐席的名字，显示在坐席主页和聊天页面中。

-   姓：坐席姓，坐席的行事，显示在坐席主页和聊天页面中。

-   角色：选择事先设置好的角色，坐席会根据角色显示对应的所属权限。

-   外显号码：当坐席设置了外显号码，当坐席呼出时，对放话机所显示的号码以及在通话记录里实际主机主叫号码中显示的号码。
...
-   **外显号码**: 定义坐席外呼时显示的主叫号码，通常会在通话记录的实际主叫号码中显示。设置外显号码的地方有三处，分别是系统偏好，外呼任务，坐席，如果三处均设置不一样的外显号码，他们的显示关系为坐席>任务>系统偏好，坐席设置的外显号码最优先显示。如果该坐席设置了多个外显号码，显示规则受系统-外显号码轮询控制。

-   **使用浏览器工作**:  选择坐席是否使用浏览器工作，如果选择使用浏览器工作，在坐席不登录浏览器时，这个坐席将无法使用话机签入坐席组。如果不选择，坐席在不登录浏览器的情况下，可以使用话机签入坐席组。当该用户设置为“使用浏览器”时，关闭浏览器后达到指定时间后，系统会自动将其从所属群组中签出（单位：秒），当用户设置为“不使用浏览器工作”时，坐席使用话机签入坐席组，系统不会自动将其从所属坐席组中签出。
