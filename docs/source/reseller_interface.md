...
-   **自动部署密码**:指坐席使用话机部署模版进行部署，在部署成功后查看配置需要输入的密码。  
    ...
    **【话机自动部署具体操作步骤】**  
    ...
    1.在client编辑页面，设置自动部署用户名和密码  

    ![alt text](_static/images/reseller/media/image45.png)     
      
    2.在物理话机上获取MAC地址（在话机上点击菜单→状态设置，即可看到MAC地址）  
      
    3.登录client用户，进入坐席编辑页面，在【高级】标签下选择提前编写好的话机部署模板，填写话机MAC地址，保存后，重新进入编辑页面，点击【预览】按钮   
    ...
    5.（以yealink为例）登录话机ip地址，在【设置-自动更新】页面填写数据  
...
-   **短信档案**:请根据已添加的短信档案名称，选择一个短信档案作为配置客户发送短信时的路由。
