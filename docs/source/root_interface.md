...
#### b. 用户: 包含代理商、Clients、账户、角色和权限四大子模块，实现分级用户与权限管控
...
#### c. 客户数据: 包含客户标签、数据来源两大子模块，用于客户信息分类与溯源管理
...
#### d.任务管理：核心子模块为呼叫结果管理，用于规范坐席通话结果记录
...
#### e. 计费: 包含客户计费、费率计划两大子模块，实现费用核算与费率配置
...
#### f. 语音设置: 包含中继管理、DID 号码、外显号码、号码区域信息四大子模块，核心配置通话线路与号码资源
...
##### DID 号码:电信运营商提供的外线呼入接入号码，可设置不同呼入路由（如转接分机、IVR、队列等）
...
外呼档案用于为坐席选择外呼中继线，在一个外呼档案中可以配置有多个呼出路由呼入路由按顺序依次向下匹配，Client选择外呼档案后，根据匹配进行外呼。
...
邮件服务器页面用于管理电子邮件收发的管理设置，配置好的邮件服务器可以在client编辑中进行选择使用。
...
**统计报表**：root级别统计暂不显示。（如下图所示）

![alt text](_static/images/root/media/image71.png)

#### j. 监控管理: 

**实时监控**：系统用户可以实时监控呼入/呼出的状态以及整个系统下坐席的状态（如下图所示）
