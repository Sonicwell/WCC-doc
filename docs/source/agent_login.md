...
在您当前使用的浏览器地址栏中输入系统网址，按下 Enter 键即可进入登录页面。

登录账号格式为：工号 + “@client名”，密码由 client 提前预设。输入正确的账号和密码后，请勾选【进行人机身份验证】，完成验证后点击【登录】，即可成功进入系统。

如希望下次登录更便捷，可在登录时勾选【记住密码】选项，浏览器将自动保存本次输入的账号和密码。  
...
- **工单预约**：用于集中管理已被设定为“预约状态”的工单。

设置预约-坐席可为任意工单指定一个预约时间。设定后，该工单将在此处显示。

智能提醒：当工单预约时间临近时，系统将自动触发提醒。

即使关闭了客户页面，提醒窗口也会自动弹出，确保您不会错过。

具体的提前提醒时长，可在【系统偏好】的“预约提醒时间”中自定义设置。
...
**客户预约**：客户预约有快速预约和指定的时间预约两种方式

![alt text](_static/images/agent/image-a53.png)

快速预约可以通过点击黄色快速预约的时间，预约时间会根据选择的快速预约时间自动更新保存。

当预约未到时间，坐席需要修改时间，修改时间即为重新预约，并不是时间累加。

指定时间预约通过选择预约时间的文本，下拉具体时间，然后根据需要选择指定的日期和时间点击【应用】，然后再选择【更新预约时间】即可成功保存。保存后可以在仪表盘页面查看。
...
**问卷**:问卷是坐席与客户通话时坐席根据问卷内容对客户进行问卷调查,问卷需要在任务管理---话术中进行设置。问卷的答案，可以由client用户在问卷答案中进行导出。
